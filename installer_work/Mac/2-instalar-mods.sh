...
# packwiz-installer guarda cache aqui
rm -rf "$HOME/.cache/packwiz-installer" 2>/dev/null
rm -rf "$HOME/.cache/packwiz" 2>/dev/null
rm -rf "$HOME/Library/Caches/packwiz-installer" 2>/dev/null
rm -rf "$HOME/Library/Caches/packwiz" 2>/dev/null
...
# --- Sincronizar mods (limpia viejos para evitar huerfanos) ---
echo "Sincronizando mods..."
rm -f "$MODS_DEST"/*.jar 2>/dev/null
...
if [ -d "$TEMP_DIR/shaderpacks" ]; then
    ...
    # --- Limpiar shaderpacks viejos ---
    rm -f "$INSTANCE_DIR/shaderpacks"/* 2>/dev/null
